...
function createKeyDetails(key: any, padding: number): crypto.RsaPublicKey | crypto.RsaPrivateKey | null {
...
export
type PublicKey = string | crypto.RsaPublicKey

export
type PrivateKey = string | crypto.RsaPrivateKey
...
export 
function makeRSACryptoWith(pair: RsaPair) {
  ...
  function publicEncrypt(message: string): Buffer {
    if(!publicKey) throw Error('Public Key required.');
  ...
  function privateDecrypt(message: string): Buffer {
    if(!privateKey) throw Error('Private Key required.');
    const messageBuffer = Buffer.from(message, 'base64');
